...
'''
Hover over Mandelbrot to view corresponding Julia!
This uses multiprocessing but is much slower than the non-MP variant for lower resos.
This scales much better tho, so it's better for higher resos/iters'''
...
# hovering auto generation and LMB functionality
# one problem! multiprocessing likely slows this down for smaller resos
...
def update_screen(width_range = None, height_range = None, GENERATE_BROT=True, julia_on_alpha=True):
	...
	with (
		...
		rect_data_q = m.Queue()
		optimise_cardioid = Z_POWER == 2 and C_POWER == 1 and GENERATE_BROT and not COLOUR_SET and corners_in_c_or_b(width_range, height_range, topleft, graph_width, graph_height)
		
		update_surf = pygame.Surface((width_range[1] - width_range[0], height_range[1] - height_range[0]))
		if not GENERATE_BROT and julia_on_alpha:
			update_surf.set_alpha(JULIA_ALPHA)
		update_topleft = width_range[0], height_range[0]
		update_array = np.zeros(update_surf.get_size(), dtype=int)
