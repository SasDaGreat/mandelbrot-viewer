...
def get_colour(x,y):
	c = complex(x,y)
	#if c == 0+0j: return getrgb(f"hsv({disn(0)},100%,80%)") 		# ENABLE ONLY WHEN GENERATING INVERSE MANDELBROTS
	z = INITIAL_Z
	dzBYdc = 0+0j
...
while 1:
	...
	mandel_img.save(f"mandelpilv5_{POWER}_{ESCAPE_LIMIT}_{INITIAL_Z}_{COLOURS}_{iters},{topleft}_{graph_width}_{graph_height}.png")
