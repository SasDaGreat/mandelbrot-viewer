...
def update_screen(width_range, height_range, horizontal_scrolling):
	...
	pygame.image.save(screen,f"mandelpygamev9.1d pz{Z_POWER},c{C_POWER} i{iters} h{int(INV_HUE_OFF)} c{COLOURS} ER{ESCAPE_RADIUS} z{INITIAL_Z}, tl{topleft} w{graph_width} h{graph_height}.png")
...
def normalise(i,modulus):
	try:
		return round(i - ln(ln(modulus), Z_POWER), 3)
	except ValueError:
		return round(ln(i/modulus)**2.5, 3) if modulus != 0 else 0
...
def get_colour(x,y):
	...
	for i in range(iters):
		...
		z_mod = abs(z)
		if z_mod > ESCAPE_RADIUS: break
	
	return (get_hue( normalise(i,z_mod) ),100,100,100)
