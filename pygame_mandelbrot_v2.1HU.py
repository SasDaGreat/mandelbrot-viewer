...
# there are two planes: the pygame graph and the complex plane graph, which is not centered at 0 once zoomed in.
...
MAX_ITERS = 40
# default = 800,600; 1080p = 1920,1080; 
DISPLAY   = WIDTH,HEIGHT = 1920,1080
...
# NEW FEATURE: HUE_SHIFT controls by how much the hue of the starting colour value is shifted,
#  to get a totally different colour range! value can be from 0 to 255.
HUE_SHIFT = 150


# number of iterations will probably be scaled once i get to 
no_of_iterations = 4500
...
while 1:
		...
		for pygame_y in range(HEIGHT+1):
			...
			for current_iteration in range(no_of_iterations):
				...
				if abs(z) > 2:
					...
					#hue_value = ( (1 - ((current_iteration % MAX_ITERS)/MAX_ITERS)) * 255  +  HUE_SHIFT )%256
					hue_value = ( ((current_iteration % MAX_ITERS)/MAX_ITERS) * 255  +  HUE_SHIFT )%256	
					# if HUE_SHIFT = 0, 1 - fraction for bluish background, fraction only for reddish background (cooler imo :/)
	...
	pygame.image.save(screen,f"mandelbrotv2.1hu_{POWER}_{MAX_ITERS}_{HUE_SHIFT}_{no_of_iterations}_{topleft_coords.real}x{topleft_coords.imag}y_{length_scale_ratio}_{actual_height}.png")
