...
'''
New features over v4.3:
- full vectorisation with numpy (doesn't improve performance, though, since some processes might exit early)
- unintentional colour bug, leading to a pretty cool colouring scheme'''
...
# doesn't have features of v4.4PAN like zooming and panning!
...
def calc_rect(data_tup):
	...
	try:
		...
		x_range = get_graph_x(start_pg_x, topleft[0], graph_width), get_graph_x(end_pg_x, topleft[0], graph_width)
		y_range = get_graph_y(height_range[0], topleft[1], graph_height), get_graph_y(height_range[1], topleft[1], graph_height)

		width_diff = end_pg_x - start_pg_x
		...
		rect_data_q.put((iter_arr, start_pg_x))
...
def update_screen(width_range, height_range, moved=False):
	...
	with (
		...
		datalist = []
		borders = np.linspace(*width_range, num=CPUS+1, dtype=int)
		last_border = borders[0]
		for border in borders[1:]:
			datalist.append(data + (int(last_border), int(border)))
			last_border = border
		...
		while running:
			...
			try:
				...
				if ENABLE_TRULY_AMAZING_COLOUR_SCHEME:
					...
					surf = pygame.surfarray.make_surface(rect_data[0])
					# can enable this instead for ANOTHER colouring scheme! there's tons to try out!!
					# surf = pygame.surfarray.make_surface(abs(rect_data[0] - 2**31))
				else:
					surf = pygame.Surface(rect_data[0].shape)
					pygame.surfarray.blit_array(surf, rect_data[0])
				pygame.display.update(screen.blit(surf, (rect_leftx, height_range[0])))					
			except Empty:
				if tasks_done: break
				else: continue
...
if __name__ == "__main__":
	...
	while 1:
		...
		printf(f"\nDone! Took {elapsed} seconds.\n\nLeft click to select the new topleft coordinate, right click to show coordinates of a point, use arrow keys to move the graph, or S to save image.")
