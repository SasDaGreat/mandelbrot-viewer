...
while 1:
	...
	for pygame_x in range(WIDTH+1):
		...
		for pygame_y in range(HEIGHT+1):
			...
			for current_iteration in range(no_of_iterations):
				...
				if abs(z) > 2: #1e21
					# now, map iterations exited at to a number in the range 0-255 for a hue value >< NO! 0 TO 360 YOU SHIT
					...
					#hue_value = (1 - ((current_iteration % MAX_ITERS)/MAX_ITERS)) * 361
					hue_value = ((current_iteration % MAX_ITERS)/MAX_ITERS) * 361
	...
	pygame.image.save(screen,f"mandelbrotzoomv2.2_{POWER}_{MAX_ITERS}_{no_of_iterations}_{topleft_coords.real}x{topleft_coords.imag}y_{length_scale_ratio}_{actual_height}.png")
