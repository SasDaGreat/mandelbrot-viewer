...
# PANNING ADDITIONS
# - S to save screen
# - screen updates once rmb is released
# - when zooming (only when rmb), overlay a resized ver
# - drag lmb to zoom
# - also added log

# - also added Julia func
...
def save_img():
	...
	meta.add_text("extras", f"v4.4 interlaced pan")
...
if __name__ == "__main__":
	...
	if len(argv) > 1: 
		...
		WIDTH, HEIGHT = 500, 400							# default: 1200, 900 OR 800, 600
		Z_POWER = 2
		C_POWER = 1
	...
	while 1:
		...
		printf(f"\nDone! Took {elapsed} seconds.\n\nDrag left click to zoom, right click to pan around the graph, scroll wheel to zoom (refresh by panning), S to save image.")
				...
				if inputs_left == 3 and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
					...
					if event.button == 1 and inputs_left == 2:
						...
						if not has_zoomed:
							...
							update_screen(width_range_hor, (0, HEIGHT)), update_screen(width_range_ver, height_range_ver)
