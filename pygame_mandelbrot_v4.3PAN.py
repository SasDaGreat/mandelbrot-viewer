...
'''
v4.3 but with panning! Back-updated from v4.4i_PAN'''
...
def get_colour(x, y_arr, iters, optimise_cardioid):
	...
	# everything after this uses c_arr.shape, coz v4.4 (full vectorisation ver) will use 2D c_arr
...
def update_screen(width_range, height_range, moved=False):
	...
	with (
		...
		optimise_cardioid = Z_POWER == 2 and C_POWER == 1 and not COLOUR_SET and INITIAL_Z == complex(0,0) and GENERATE_BROT and corners_in_c_or_b(width_range, height_range, topleft, graph_width, graph_height)
...
def update_pause_screen(circle_coords, circle_colour, inputs_left, topleft=None, rmb_held=False):
	# if in the first stage, allow panning
	screen.blit(bg_display, pan_amount if inputs_left == 3 else (0,0))
...
if __name__ == "__main__":
	...
	if len(argv) > 1:
		...
		WIDTH, HEIGHT = 600, 500							# default: 1200, 900 OR 800, 600
		Z_POWER = 2
		C_POWER = 1
	...
	pygame.display.set_caption(f"The Mandelbrot Set" if GENERATE_BROT else "The Julia Set")
	...
	while 1:
		...
		while 1:
			...
			pygame.display.set_caption(f"The Mandelbrot Set @ power {Z_POWER}, {iters} iterations" if GENERATE_BROT else f"The Julia Set @ {iters} iterations")
		...
		while inputs_left:
			for event in pygame.event.get():
				if inputs_left == 3 and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
					screen.blit(bg_display, (0,0))
				...
				elif inputs_left == 3 and event.type == pygame.MOUSEWHEEL:
					has_zoomed = True

					# event.y = -1 for zoom out, +1 for zoom in
					topleft = (
						topleft[0] + graph_width * ZOOM_PERCENT/2 * event.y,
						topleft[1] - graph_height * ZOOM_PERCENT/2 * event.y,
						)
					graph_width *= 1 - ZOOM_PERCENT * event.y
					graph_height *= 1 - ZOOM_PERCENT * event.y

					zoomed_display = pygame.transform.smoothscale_by(bg_display, 1 + ZOOM_PERCENT * event.y)
					zoomed_rect = zoomed_display.get_rect()
					zoomed_rect.center = WIDTH//2, HEIGHT//2
					screen.fill(SCREEN_BG_COLOUR)
					screen.blit(zoomed_display, zoomed_rect)
					bg_display = screen.copy()
					pygame.display.flip()

				elif event.type == pygame.MOUSEBUTTONUP:
					if event.button == 1 and inputs_left == 2:
					...
					elif event.button == 3 and inputs_left == 3:
						screen.blit(bg_display, pan_amount)

						topleft = (
							topleft[0] - pan_amount[0] / WIDTH * graph_width,
							topleft[1] + pan_amount[1] / HEIGHT * graph_height,
							)

						alpha_layer.fill(MANDEL_COLOUR + (150,))
						if not has_zoomed:
							if pan_amount[0] > 0:
								# if moved mouse to right
								width_range_hor = (0, pan_amount[0])
								width_range_ver = (pan_amount[0], WIDTH)
							else:
								# left
								width_range_hor = (WIDTH + pan_amount[0], WIDTH)
								width_range_ver = (0, WIDTH + pan_amount[0])
							
							if pan_amount[1] > 0:
								# if moved mouse down
								height_range_ver = (0, pan_amount[1])
							else:
								# up
								height_range_ver = (HEIGHT + pan_amount[1], HEIGHT)

							# don't ask what i did here, I'm doing this at 6 in the morning with no sleep for no reason help help help
							screen.blit(alpha_layer, (width_range_hor[0], 0), (0,0, abs(pan_amount[0]), HEIGHT))
							screen.blit(alpha_layer, (width_range_ver[0], height_range_ver[0]), (0,0, WIDTH - abs(pan_amount[0]), abs(pan_amount[1])))
							pygame.display.flip()
							
							update_screen(width_range_hor, (0, HEIGHT), moved=True), update_screen(width_range_ver, height_range_ver, moved=True)
							print(f"Panned! New topleft: {topleft}")
						else:
							screen.blit(alpha_layer, (0,0))
							pygame.display.flip()

							update_screen((0, WIDTH), (0, HEIGHT))
							print(f"Panned! New topleft: {topleft}; zoomed dimensions: {graph_width} by {graph_height}")
							has_zoomed = False
					...
					match inputs_left:
						case 3:
							# if RMB is being held, add to the pan counter until released
							if pygame.mouse.get_pressed()[2]:
								pan_amount[0] += event.rel[0]
								pan_amount[1] += event.rel[1]
							update_pause_screen(event.pos, CIRCLE1_COLOUR, inputs_left, rmb_held=pygame.mouse.get_pressed()[2])
						case 2:
