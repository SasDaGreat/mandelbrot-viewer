...
# - the fitness gram pacer test is an
...
def update_screen(width_range, height_range, horizontal_scrolling):
	...
	pygame.image.save(screen,f"mandelpygamev9 pz{Z_POWER},c{C_POWER} i{iters} ER{ESCAPE_RADIUS} z{INITIAL_Z}, tl{topleft} w{graph_width} h{graph_height}.png")
...
def get_colour(x,y):
	if not COLOUR_SET and in_cardioid_or_bulb(x,y): return MANDEL_COLOUR

	c = complex(x,y)
	if C_POWER < 0 and c == 0+0j: return (0,100,80,100) 	# ENABLE ONLY FOR INVERSE MANDELBROTS
	z = INITIAL_Z

	for i in range(iters):
		z = z**Z_POWER + c**C_POWER
		z_mod = abs(z)
		if z_mod > ESCAPE_RADIUS: return ( (phase(z)+pi)/(2*pi) * HUE_RANGE, 100, 100, 100 ) if COLOUR_OUT else MANDEL_COLOUR

	return ( (phase(z)+pi)/(2*pi) * HUE_RANGE, 100, 100, 100 ) if COLOUR_SET else MANDEL_COLOUR
