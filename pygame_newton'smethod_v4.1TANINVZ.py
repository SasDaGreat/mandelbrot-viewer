...
def update_screen(width_range, height_range, horizontal_scrolling):
	...
	pygame.image.save(screen,f"newtonpygamev4 arctanz i{iters} h{int(INV_HUE_OFF)} c{COLOURS} ER{ESCAPE_RADIUS} z{INITIAL_Z}, tl{topleft} w{graph_width} h{graph_height}.png")
...
def get_colour(x,y):
	...
	for i in range(iters):
		...
		if z_mod > ESCAPE_RADIUS: return (get_hue( i ),100,100,100)
	else:
		return ( (phase(z)+pi) * RAD_TO_DEG, 100, 100, 100 )
...
while 1:
	pygame.display.set_caption(f"Using Newton's method to find arctan(z), {iters} iterations")
