...
def get_colour(x,y):
	c = complex(x,y)
	if C_PW < 0 and  c == 0+0j: return disn(0,0,False) 		# ENABLE ONLY WHEN GENERATING INVERSE MANDELBROTS
	z = INITIAL_Z
	dzBYdc = 0+0j
...
# actual graph shit:
WIDTH,HEIGHT= 800,450			# default: 800 by 600
graph_width	= 8.5 				# default: 4
graph_height= get_graph_height(graph_width)
topleft		= (-graph_width/2 + 2, graph_height/2)
...
while 1:
	...
	mandel_img.save(f"mandelpilv5.1INV_{MOD_BY}_{POWER}_{C_PW}_{ESCAPE_LIMIT}_{INITIAL_Z}_{COLOURS}_{iters},{topleft}_{graph_width}_{graph_height}.png")
