...
def disn(abs_z,abs_dz, escaped):
	if abs_z==0: return (RGB_RANGE-1, RGB_RANGE-1, RGB_RANGE-1)
	
	dis_inv = abs_dz/(abs_z * ln(abs_z));
	frac 	= (dis_inv%COLOURS)/COLOURS
	
	if escaped:
		shade = frac * HUE_RANGE
		return getrgb(f"hsv({shade},100%,100%)")

	shade = int((1 - frac) * MAX_RGB)
...
def get_colour(x,y):
	...
	for i in range(iters):
		try:
			dzBYdc = POWER * (z**P_MINUS_1) * dzBYdc + C_POWER * c**C_P_MINUS_1
			z = z**POWER + c**C_POWER
			dzBYdc -= dzBYdc/z
			z += DEFORMED/z
...
P_MINUS_1 	= POWER-1
C_POWER 	= 1
C_P_MINUS_1	= C_POWER - 1 
LN_P 		= ln(POWER)
DEFORMED 	= 0.00005
ESCAPE_LIMIT= 10e20 				# default: 10
...
# actual graph shit:
WIDTH,HEIGHT= 480,128			# default: 800 by 600
graph_width	= 0.002295356156843755 				# default: 4
graph_height= get_graph_height(graph_width)
topleft		= (-0.02428581349904687, 0.999311827034469)
...
while 1:
	...
	mandel_img.save(f"mandelpilv5DEFORMED{DEFORMED}_{POWER}_{C_POWER}_{ESCAPE_LIMIT}_{INITIAL_Z}_{COLOURS}_{iters},{topleft}_{graph_width}_{graph_height}.png")
