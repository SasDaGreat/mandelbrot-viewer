...
def update_screen(width_range, height_range, horizontal_scrolling):
	...
	pygame.image.save(screen,f"mandelpygamev5.1DEF{DEFORMED} pz{Z_POWER},c{C_POWER} i{iters} h{int(INV_HUE_OFF)} c{COLOURS} c{int(COLOUR_CAP)} ER{ESCAPE_RADIUS} z{INITIAL_Z}, tl{topleft} w{graph_width} h{graph_height}.png")
...
def disn(abs_z,abs_dz, escaped):
	if abs_z==0: return (0, 0, 100, 100)
	
	dis 	= (abs_z * ln(abs_z))/abs_dz
	frac 	= dis * COLOURS
	
	if escaped:
		if COLOUR_CAP and frac >= 1: return (HUE_RANGE, 100, 100, 100)

		frac = frac%MOD_BY * 2
		shade = int(frac * HUE_RANGE * 100)/100 if frac<1 else HUE_RANGE-int((frac-1) * HUE_RANGE * 100)/100
		return (shade, 100, 100, 100)

	frac = (frac%1) * 2
	shade = int( frac * MAX_SHADE * 10)/10 if frac<1 else MAX_SHADE - int((frac-1) * MAX_SHADE * 10)/10
	return (0, 0, shade, 100)

def get_graph_height(graph_w): return (HEIGHT/WIDTH) * graph_w

def get_colour(x,y):
	c = complex(x,y)
	if C_POWER < 1 and c == 0+0j: return (get_hue(0),100,80,100) 	# ENABLE ONLY FOR INVERSE MANDELBROTS
	z = INITIAL_Z
	dzBYdc = 0

	for i in range(iters):
		try:
			dzBYdc = Z_POWER * (z**ZP_MINUS_1) * dzBYdc + C_POWER * c**CP_MINUS_1
			z = z**Z_POWER + c**C_POWER
			dzBYdc -= dzBYdc/z
			z += DEFORMED/z
		except ZeroDivisionError:
			return disn(0,0,False)

		z_mod = abs(z)
		if z_mod > ESCAPE_RADIUS: return disn( z_mod, abs(dzBYdc), True )
	else:
...
HUE_RANGE   = 360
MAX_SHADE 	= 100
COLOURS 	= 4					# doesn't limit to this many colours, however, because of fractional iteration
...
Z_POWER		= 2
ZP_MINUS_1 	= Z_POWER - 1
C_POWER		= 1
CP_MINUS_1 	= C_POWER - 1
DEFORMED 	= 0.00005
ESCAPE_RADIUS = 10
