...
def update_screen(width_range, height_range, horizontal_scrolling):
	...
	pygame.image.save(screen,f"mandelpygamev11NABIL imag s{COLOUR_SENS} pz{Z_POWER},c{C_POWER} i{iters} h{int(INV_HUE_OFF)} c{COLOURS} ER{ESCAPE_RADIUS} z{INITIAL_Z}, tl{topleft} w{graph_width} h{graph_height}.png")
...
def get_hue(z_part,sens):
	try: return (ln(abs(z_part),10) * sens)%HUE_RANGE
	except: return 0
...
def get_colour(x,y):
	...
	for i in range(iters):
		...
		z_mod = abs(z)
		if z_mod > ESCAPE_RADIUS: return (get_hue(z.imag,COLOUR_SENS),100,100,100)
	else:
		return (get_hue(z.imag,COLOUR_SENS*5),100,100,100) if COLOUR_SET else MANDEL_COLOUR
